...
public class MyBatisTest {
    ...
    public void test01() {
        try(SqlSession sqlSession = sqlSessionFactory.openSession()){
            ...
            List<Dept> depts2 = mapper.selectDept(dept); // 调用了两次select 实际只去数据库查询一次，因为调用了缓存
     ...
     *      3.如果希望其他mapper映射文件的命名空间执行了增删改清空另外的命名空间就可以设置：  （只能清空一个命名空间，可能造成脏读）
     *          <cache-ref namespace="com.ustc.maven.mapper.DeptMapper"/>
     */
    @Test
    public void test02() {
        try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
